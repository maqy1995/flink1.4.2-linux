...
public abstract class DataSet<T> {
	...
	//maqy
	/**
	 * A ImplementPickOut represents a collection of elements of the same type.
	 *
	 * <p>A ImplementPickOut can be transformed into another DataSet by applying a transformation as for example
	 * <ul>
	 *   <li>{@link DataSet#map(org.apache.flink.api.common.functions.MapFunction)},</li>
	 *   <li>{@link DataSet#reduce(org.apache.flink.api.common.functions.ReduceFunction)},</li>
	 *   <li>{@link DataSet#join(DataSet)}, or</li>
	 ...
	 *
	 */
	public static final class ImplementPickOut implements FilterFunction<Integer>{
		int start;
		int end;
		public ImplementPickOut(int start, int end){
			this.start = start;
			this.end = end;
		}
	 ...
	 * which the function returns false are filtered.
	 *
	 * @param start The FilterFunction that is called for each element of the DataSet.
	 * @param end The FilterFunction that is called for each element of the DataSet.
	 * @return A FilterOperator that represents the filtered DataSet.
	 *
	 * @see org.apache.flink.api.common.functions.RichFilterFunction
	 * @see FilterOperator
	...
	public FilterOperator<T> pickOut(int start, int end) {
		if (start > end){
			throw new RuntimeException("the end is less than start!");
		}
		FilterFunction filter = new ImplementPickOut(start, end);
		if (filter == null) {
			throw new NullPointerException("Filter function must not be null.");
		}
		return new FilterOperator<>(this, clean(filter), Utils.getCallLocationName());
