...
class TaskManager(
    ...
    //封装了一些TaskManager的连接信息，resourceID,hostName等
    protected val location: TaskManagerLocation,
    // MemoryManager 统一管理了 flink 的内存使用，内存被划分为相同大小的 segment，
    // 通过申请不同数量的 segment 来分配不同大小的内存
    protected val memoryManager: MemoryManager,
    //IOManager 来控制磁盘 IO 的过程，提供同步和异步两种写模式【其实只有异步】，
    // 具体的读写方式又分为 block、buffer、bulk 三种方式；用户可以指定 IO 的文件目录集合，
    // IOManager 会以 round-robin 的方式写不同目录的不同文件。
    protected val ioManager: IOManager,
    //NetworkEnvironment 是每个 Instance 的网络 IO 组件
    // ，包含了追踪中间结果和数据交换的数据结构。
    // 它的构造器会统一将配置的内存先分配出来，抽象成 NetworkBufferPool 统一管理内存的申请和释放。
      ...
      case TriggerTaskManagerRegistration(
        ...
        if (registrationRun.equals(this.currentRegistrationRun)) {
          if (isConnected) {
            ...
            //maqy add
            log.info("maqy add :" + location +"location hostname" + location.getHostname)
